...
const Relevancy = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/program-selection/search-filenames`, { prompt: input });
  ...
  const handleViewClick = async (fileName) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/program-selection/get-quick-view`, {
        filename: fileName,
