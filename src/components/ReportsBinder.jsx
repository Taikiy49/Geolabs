...
export default function ReportsBinder() {
  ...
  const doDelete = async (id) => {
    ...
    try {
      ...
      setToastTimed("🗑️ Removed", null, null);
      fetchRows();
    } catch (e) {
  ...
  const bulkDelete = async () => {
    ...
    try {
      await axios.post(`${API_URL}/api/reports-binder/bulk-delete`, { ids: Array.from(selected) });
      setSelected(new Set());
      setToastTimed("🗑️ Removed selected", null, null);
      fetchRows();
    } catch (e) {
        ...
        <div className="rb-filters">
          ...
          <label className="rb-checkbox">
            <input
              type="checkbox"
              checked={billingOnly}
              onChange={(e) => { setBillingOnly(e.target.checked); setPage(1); }}
            />
